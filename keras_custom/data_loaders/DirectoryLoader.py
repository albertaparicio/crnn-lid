...
class DirectoryLoader(object):
    def __init__(self, source_directory, config, shuffle=True):
        ...
        self.generators = [
            SpectrogramGenerator(
                os.path.join(self.source_directory, "english"), config, shuffle=shuffle
            ),
            SpectrogramGenerator(
                os.path.join(self.source_directory, "german"), config, shuffle=shuffle
            ),
            SpectrogramGenerator(
                os.path.join(self.source_directory, "french"), config, shuffle=shuffle
            ),
            SpectrogramGenerator(
                os.path.join(self.source_directory, "spanish"), config, shuffle=shuffle
            ),
        ]
        ...
        self.generator_queues = [
            SpectrogramGen.get_generator() for SpectrogramGen in self.generators
        ]
    ...
    def get_data(self):
        ...
        while True:
            ...
            if self.shuffle:
                sample_selection = [
                    random.randint(0, num_classes - 1) for r in range(config["batch_size"])
                ]
            else:
            ...
            data_batch = np.zeros(
                (self.config["batch_size"],) + tuple(self.config["input_shape"])
            ...
            for i, label in enumerate(sample_selection):
                ...
                label_batch[i, :] = to_categorical(
                    [label], nb_classes=self.config["num_classes"]
                )  # one-hot encoding
...
if __name__ == "__main__":
    ...
    a = DirectoryLoader(
        "/Users/therold/Downloads/Speech Data/EU Speech",
    ...
    for (data, labels) in a.get_data():
        ...
        for image in np.vsplit(data, 32):
            scipy.misc.imsave(
                "/Users/therold/Downloads/Speech Data/EU Speech/png/%s.png" % i,
                np.squeeze(image) * 255.0,
            )
            i += 1
