...
from flask_extensions import send_file_partial, request

lib_path = os.path.abspath(os.path.join("../keras_custom"))
sys.path.append(lib_path)
from data_loaders.SpectrogramGenerator import SpectrogramGenerator


static_assets_path = os.path.join(os.path.dirname(__file__), "dist")
...
app._model = keras.models.load_model("model/2017-01-31-14-29-14.CRNN_EN_DE_FR_ES_CN_RU.model")
...
def send_audio(audio_path):
    return send_file_partial(os.path.join(app.config["UPLOAD_FOLDER"], audio_path))


@app.route("/api/upload", methods=["POST"])
def uploadAudio():
    def is_allowed(filename):
        return len(list(filter(lambda ext: ext in filename, ["wav", "mp3", "ogg"]))) > 0
...
def use_example(example_id):
    if example_id <= 6:
        ...
        file_path = os.path.join(app.config["UPLOAD_FOLDER"], "examples", filename)
        response = jsonify(get_prediction(file_path))
    else:
...
def get_prediction(file_path):
    ...
    LABEL_MAP = {0: "English", 1: "German", 2: "French", 3: "Spanish", 4: "Chinese", 5: "Russian"}

    config = {"pixel_per_second": 50, "input_shape": [129, 500, 1], "num_classes": NUM_LANGUAGES}
    data_generator = SpectrogramGenerator(
        file_path, config, shuffle=False, run_only_once=True
    ).get_generator()
    ...
    timesteps_with_labels = {
        LABEL_MAP[index]: prob for index, prob in enumerate(probabilities.T.tolist())
    }

    # transform results a little to make them ready for JSON conversion
    file_path_cachebuster = file_path + "?cachebuster=%s" % time.time()
    result = {
        "audio": {"url": "%s" % file_path_cachebuster},
...
if __name__ == "__main__":
    app.config.update(
        DEBUG=True, SECRET_KEY="asassdfs", CORS_HEADERS="Content-Type", UPLOAD_FOLDER="audio"
    )
    # Start the server
