...
def convert(filename):
    ...
    if not os.path.exists(new_filename):
        command = 'ffmpeg -i "{}" -ac 1 "{}"'.format(
            escape_characters(filename), escape_characters(new_filename)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path", dest="path", help="Directory for the files to convert", required=True
    )
